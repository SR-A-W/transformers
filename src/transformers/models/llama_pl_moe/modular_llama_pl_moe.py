...
"""PyTorch LLaMA Path-Locked MoE (PL-MoE) model.
...
与 Qwen3PlMoe 的区别：
- LLaMA attention 无 q_norm/k_norm
...
class LlamaPlMoeExpertMLP(LlamaMLP):
    """单个 MLP 专家（与 LlamaMLP 结构完全一致），用于 PL-MoE 的双专家容器。
...
class LlamaPlMoeMLP(nn.Module):
    ...
    def __init__(self, config: LlamaPlMoeConfig):
        super().__init__()
        self.experts = nn.ModuleList([LlamaPlMoeExpertMLP(config) for _ in range(2)])
...
def _determine_routing_index(input_ids: torch.LongTensor, config: LlamaPlMoeConfig) -> int:
...
class LlamaPlMoeDecoderLayer(LlamaDecoderLayer):
    """Decoder 层：Attention 与 LLaMA 一致，MLP 替换为 LlamaPlMoeMLP，forward 透传 routing_index。
    ...
    def __init__(self, config: LlamaPlMoeConfig, layer_idx: int):
        super().__init__(config=config, layer_idx=layer_idx)
        # 将父类创建的 LlamaMLP 替换为 PL-MoE 双专家容器
        self.mlp = LlamaPlMoeMLP(config)
...
class LlamaPlMoeModel(LlamaModel):
    ...
    def __init__(self, config: LlamaPlMoeConfig):
        ...
        # 将父类创建的 LlamaDecoderLayer 替换为 LlamaPlMoeDecoderLayer（双专家 MLP + routing_index）
        self.layers = nn.ModuleList(
            [LlamaPlMoeDecoderLayer(config, layer_idx=i) for i in range(config.num_hidden_layers)]
        )
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        #   - generate 阶段：由 LlamaPlMoeForCausalLM.prepare_inputs_for_generation 在首步计算并缓存
...
class LlamaPlMoeForCausalLM(LlamaForCausalLM):
    """Causal LM 顶层：使用 LlamaPlMoeModel 作为 backbone。
    ...
    def __init__(self, config: LlamaPlMoeConfig):
        super().__init__(config)
        self.model = LlamaPlMoeModel(config)
    ...
    def prepare_inputs_for_generation(
        ...
        **完整的累积序列**（原始 prompt + 已生成的 token），而非只有新 token。
        ...
        # 注入 routing_index，使其透传到 LlamaPlMoeModel.forward 的 kwargs 中
