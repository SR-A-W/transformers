...
"""LLaMA Path-Locked MoE (PL-MoE) model configuration."""
...
class LlamaPlMoeConfig(LlamaConfig):
    r"""
    Path-Locked MoE 配置类，继承自 LlamaConfig。

    在 LLaMA 基础上增加路由相关配置，用于序列级硬路由：
    根据 input_ids 中出现的控制 token 决定整条序列走哪个 expert。

    与 Qwen3PlMoeConfig 的主要区别：
